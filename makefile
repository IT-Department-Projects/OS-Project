...
OUTPUT = OsBootable/boot/kernel.bin
...
build:all
	rm OsBootable/boot/grub/ -r -f
	mkdir OsBootable/boot/grub/
	echo set default=0 >> OsBootable/boot/grub/grub.cfg
	echo set timeout=0 >> OsBootable/boot/grub/grub.cfg
	echo menuentry "IKnow" { >> OsBootable/boot/grub/grub.cfg
	echo         set root='(hd96)' >> OsBootable/boot/grub/grub.cfg
	echo         multiboot /boot/kernel.bin >> OsBootable/boot/grub/grub.cfg
	echo } >> OsBootable/boot/grub/grub.cfg

	grub-mkrescue -o OsBootable.iso OsBootable/
